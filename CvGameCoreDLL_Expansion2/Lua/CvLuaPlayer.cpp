...
#if defined(MOD_API_LUA_EXTENSIONS) && defined(MOD_LUA_AI_DEAL_FETCHINFO)
    // eg used in TradeLogic.lua to show the AI DEAL value
...
int CvLuaPlayer::lGetTotalValueToMeNormal(lua_State* L)
{
	...
	CUSTOMLOG("lGetTotalValueToMeNormal iResult %i",iResult);
...
int CvLuaPlayer::lGetTotalValueToMe(lua_State* L)
{
	...
	int iResult = 0;
	iResult = pkThisPlayer->GetDealAI()->GetDealValue(pkDeal, iValueImOffering, iValueTheyreOffering, false);
	CUSTOMLOG("lGetTotalValueToMe iResult %i",iResult);
    if(iResult < 0)
	{
		iResult *= -1;
	}

...
int CvUnit::GetStrategicResourceCombatPenalty() const
{
...
#if defined(MOD_POSITIVE_STRAT_COMBAT)
    if(MOD_POSITIVE_STRAT_COMBAT)
    {
        // we sum up, to be compatible to unit consuming several different ressources
        int iRequiredTotal = 0;
        int iUsedTotal = 0;
        int iAvailableTotal = 0;
        for(int iResourceLoop = 0; iResourceLoop < iNumResourceInfos; iResourceLoop++)
        {
            eResource = (ResourceTypes) iResourceLoop;
            int iRequired = m_pUnitInfo->GetResourceQuantityRequirement(eResource);
            if (iRequired > 0)
            {
                ...
                if (iAvailable<0)
                    iAvailable *= 2; // double negative values, so it is not easy to replace one ressource by another, if multiple are needed
                iAvailableTotal += iAvailable;
                iRequiredTotal += iRequired;
            }
        }
        if(iRequiredTotal > 0 && iAvailableTotal != 0)
        {
            if(iAvailableTotal > 0)
            {
                double dDeficit = (double) iAvailableTotal / (double) iUsedTotal;
                iPenalty += (int) ceil(-1* (dDeficit) * (GC.getSTRATEGIC_RESOURCE_EXHAUSTED_PENALTY()/5)); // round up
                iPenalty = min(iPenalty, -1*GC.getSTRATEGIC_RESOURCE_EXHAUSTED_PENALTY());
            }
            else if(iAvailableTotal < 0)
            {
                double dAvailableTotal = (double) iAvailableTotal / 2;
                double dDeficit = -1 * ((double) dAvailableTotal / (double) iUsedTotal);
    ...
    // ORIGINAL code that does not work well with different start ressources for same unit
    else
    {
        for(int iResourceLoop = 0; iResourceLoop < iNumResourceInfos; iResourceLoop++)
        {
...
void CvUnit::PushMission(MissionTypes eMission, int iData1, int iData2, int iFlags, bool bAppend, bool bManual, MissionAITypes eMissionAI, CvPlot* pMissionAIPlot, CvUnit* pMissionAIUnit)
{
    ...
    bool isanyplayerdisconnected = false;
    for (int iI = 0; iI < MAX_MAJOR_CIVS; iI++)
		{
			CvPlayer& kPlayer = GET_PLAYER((PlayerTypes)iI);
			if (kPlayer.isAlive() && kPlayer.isHuman() && kPlayer.isDisconnected())
			{
				isanyplayerdisconnected = true;
				break;
			}
		}
    if (  isanyplayerdisconnected && (GC.getGame().isOption(GAMEOPTION_DYNAMIC_TURNS) || GC.getGame().isOption(GAMEOPTION_SIMULTANEOUS_TURNS))) {
        return;
    }
#endif
	VALIDATE_OBJECT

...
#if defined(MOD_DEBUG_GAMEHOOKS) 
	if (MOD_DEBUG_GAMEHOOKS)
        CUSTOMLOG("CustomMods eventHook called: %s", szName);
#endif
...
#if defined(MOD_DEBUG_GAMEHOOKS) 
        if (MOD_DEBUG_GAMEHOOKS)
            CUSTOMLOG("CustomMods eventHook %s we have pkScriptSystem", szName);
#endif
		if (LuaSupport::CallHook(pkScriptSystem, szName, args.get(), bResult)) {
#if defined(MOD_DEBUG_GAMEHOOKS) 
            if (MOD_DEBUG_GAMEHOOKS)
                CUSTOMLOG("CustomMods eventHook %s return successfully event", szName);
#endif
...
#if defined(MOD_DEBUG_GAMEHOOKS) 
    if (MOD_DEBUG_GAMEHOOKS)
        CUSTOMLOG("CustomMods eventHook %s return no pkScriptSystem", szName);
#endif
...
#if defined(MOD_DEBUG_GAMEHOOKS) 
	if (MOD_DEBUG_GAMEHOOKS)
        CUSTOMLOG("CustomMods eventTestAll called: %s", szName);
#endif
    ICvEngineScriptSystem1* pkScriptSystem = gDLL->GetScriptSystem();
	if (pkScriptSystem) {
		bool bResult = false;
#if defined(MOD_DEBUG_GAMEHOOKS) 
        if (MOD_DEBUG_GAMEHOOKS)
            CUSTOMLOG("CustomMods eventTestAll %s we have pkScriptSystem", szName);
#endif
        if (LuaSupport::CallTestAll(pkScriptSystem, szName, args.get(), bResult)) {
#if defined(MOD_DEBUG_GAMEHOOKS) 
            if (MOD_DEBUG_GAMEHOOKS)
                CUSTOMLOG("CustomMods eventTestAll %s return successfully event", szName);
#endif
            if (bResult) {
...
#if defined(MOD_DEBUG_GAMEHOOKS) 
    if (MOD_DEBUG_GAMEHOOKS)
        CUSTOMLOG("CustomMods eventTestAll %s return no pkScriptSystem", szName);
#endif
	return GAMEEVENTRETURN_NONE;
}
...
#if defined(MOD_DEBUG_GAMEHOOKS) 
	if (MOD_DEBUG_GAMEHOOKS)
        CUSTOMLOG("CustomMods eventTestAny called: %s", szName);
#endif
    ICvEngineScriptSystem1* pkScriptSystem = gDLL->GetScriptSystem();
	if (pkScriptSystem) {
		bool bResult = false;
#if defined(MOD_DEBUG_GAMEHOOKS) 
        if (MOD_DEBUG_GAMEHOOKS)
            CUSTOMLOG("CustomMods eventTestAny %s we have pkScriptSystem", szName);
#endif
		if (LuaSupport::CallTestAny(pkScriptSystem, szName, args.get(), bResult)) {
#if defined(MOD_DEBUG_GAMEHOOKS) 
            if (MOD_DEBUG_GAMEHOOKS)
                CUSTOMLOG("CustomMods eventTestAny %s return successfully event", szName);
#endif
            if (bResult) {
				return GAMEEVENTRETURN_TRUE;
			} else {
...
#if defined(MOD_DEBUG_GAMEHOOKS) 
    if (MOD_DEBUG_GAMEHOOKS)
        CUSTOMLOG("CustomMods eventTestAny %s return no pkScriptSystem", szName);
#endif
	return GAMEEVENTRETURN_NONE;
}
...
#if defined(MOD_DEBUG_GAMEHOOKS) 
	if (MOD_DEBUG_GAMEHOOKS)
        CUSTOMLOG("CustomMods eventAccumulator called: %s", szName);
#endif
    ICvEngineScriptSystem1* pkScriptSystem = gDLL->GetScriptSystem();
	if (pkScriptSystem) {
#if defined(MOD_DEBUG_GAMEHOOKS) 
        if (MOD_DEBUG_GAMEHOOKS)
            CUSTOMLOG("CustomMods eventAccumulator %s we have pkScriptSystem", szName);
#endif
        if (LuaSupport::CallAccumulator(pkScriptSystem, szName, args.get(), iValue)) {
#if defined(MOD_DEBUG_GAMEHOOKS) 
            if (MOD_DEBUG_GAMEHOOKS)
                CUSTOMLOG("CustomMods eventAccumulator %s return successfully event", szName);
#endif
            return GAMEEVENTRETURN_TRUE;
		}
	}
#if defined(MOD_DEBUG_GAMEHOOKS) 
    if (MOD_DEBUG_GAMEHOOKS)
        CUSTOMLOG("CustomMods eventAccumulator %s return no pkScriptSystem", szName);
#endif
	return GAMEEVENTRETURN_NONE;
}
...
int CustomMods::getOption(string sOption, int defValue) {
	if (!m_bInit) {
        ...
        MOD_OPT_CACHE(BUGFIX_NAVAL_TARGETING);
        ...
        // Added by Serp from community patch dll or selfmade
        MOD_OPT_CACHE(ANY_PANTHEON);
        MOD_OPT_CACHE(RESOURCES_PRODUCTION_COST_MODIFIERS);
        MOD_OPT_CACHE(EVENTS_VISCOUNT_CHANGED);

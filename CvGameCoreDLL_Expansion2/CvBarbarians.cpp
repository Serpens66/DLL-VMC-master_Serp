...
bool CvBarbarians::CanBarbariansSpawn()
{
    ...
    if (kGame.getElapsedGameTurns() < GD_INT_GET(BARBARIAN_NO_SPAWN_BEFORE_TURN))
	{
		return false;
	}
#else
...
#if defined(GAMEOPTION_CHILL_BARBARIANS)
...
void CvBarbarians::DoCamps()
{
	...
	if(eCamp != NO_IMPROVEMENT)
	{
        ...
        WorldSizeTypes	MapSize = GC.getMap().getWorldSize();
        switch(MapSize)
        {
        case WORLDSIZE_DUEL:
            iCampTargetNum = max(iCampTargetNum, min(2,iNumValidCampPlots));
            break;
        case WORLDSIZE_TINY:
            iCampTargetNum = max(iCampTargetNum, min(4,iNumValidCampPlots));
            break;
        case WORLDSIZE_SMALL:
            iCampTargetNum = max(iCampTargetNum, min(6,iNumValidCampPlots));
            break;
        case WORLDSIZE_STANDARD:
            iCampTargetNum = max(iCampTargetNum, min(8,iNumValidCampPlots));
            break;
        case WORLDSIZE_LARGE:
            iCampTargetNum = max(iCampTargetNum, min(12,iNumValidCampPlots));
            break;
        case WORLDSIZE_HUGE:
            iCampTargetNum = max(iCampTargetNum, min(14,iNumValidCampPlots));
            break;
        default:
            iCampTargetNum = max(iCampTargetNum, min(6,iNumValidCampPlots));
        }
#if defined(GAMEOPTION_CHILL_BARBARIANS)
        if (kGame.isOption(GAMEOPTION_CHILL_BARBARIANS)) // only 2/3 of max number of camps
        {
            iCampTargetNum *= 66;
            iCampTargetNum /= 100;
        }
		...
		if(iNumCampsToAdd > 0 && GC.getBARBARIAN_CAMP_ODDS_OF_NEW_CAMP_SPAWNING() > 0) // slewis - added the barbarian chance for the FoR scenario
		{
			...
			do
			{
                ...
                if (vRelevantPlots.size() <= 0 || vRelevantPlots.empty()) // if we don't have any valid plots left at all, then bail
					break;
                int iPlotIndex = kGame.getJonRandNum( vRelevantPlots.size(), "Barb Camp Plot-Finding Roll");
                ...
                if(MOD_BUGFIX_BARB_CAMP_TERRAINS == false || pkImprovementInfo == NULL || (pkImprovementInfo->GetTerrainMakesValid(pLoopPlot->getTerrainType()) && pkImprovementInfo->GetFeatureMakesValid(pLoopPlot->getFeatureType()))) {
					...
					if (pLoopPlot->area()->getNumImprovements(eCamp) <= iMaxCampsThisArea)
					{
						...
						{
							CvUnit* pUnit = GET_PLAYER(BARBARIAN_PLAYER).initUnit(eBestUnit, pLoopPlot->getX(), pLoopPlot->getY(), (UnitAITypes) GC.getUnitInfo(eBestUnit)->GetDefaultUnitAIType());
							if (pUnit)
								pUnit->finishMoves();
#if defined(MOD_EVENTS_BARBARIANS)
							if (MOD_EVENTS_BARBARIANS && pUnit!=NULL) {
								GAMEEVENTINVOKE_HOOK(GAMEEVENT_BarbariansSpawnedUnit, pLoopPlot->getX(), pLoopPlot->getY(), eBestUnit,pUnit->GetID(), pLoopPlot->getX(), pLoopPlot->getY());
							}
#endif
						}
...
#if defined(GAMEOPTION_CHILL_BARBARIANS)
						if (!GC.getGame().isOption(GAMEOPTION_CHILL_BARBARIANS) && CanBarbariansSpawn())
						{
...
#else // MOD_BETTER_BARBCAMP_SPAWNCODE
...
#endif // MOD_BETTER_BARBCAMP_SPAWNCODE
		...
		if(m_aeValidBarbSpawnDirections.size() > 0)
		{
			...
			if(eUnit != NO_UNIT)
			{
				...
				if (pUnit != NULL && MOD_EVENTS_BARBARIANS) {
					GAMEEVENTINVOKE_HOOK(GAMEEVENT_BarbariansSpawnedUnit, pSpawnPlot->getX(), pSpawnPlot->getY(), eUnit, pUnit->GetID(), pPlot->getX(), pPlot->getY());
				}
#endif
			}
		}

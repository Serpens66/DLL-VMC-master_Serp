 			...
 			for(int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
			{
				...
				if(GET_PLAYER(ePlayer).getTeam() == eTeam)
				{
					...
					if (MOD_GLOBAL_CS_GIFTS) {
						...
						// if (eMeetingPlayer == ePlayer) { // serp: this is bullshit as long eMeetingPlayer is always the team leader -.- so we will give everything to all team members, but no one gets iExtraFriendship
							...
							if (eTrait == MINOR_CIV_TRAIT_MILITARISTIC) {
								if (iUnitGift > 0) {
									if (GC.getGame().getJonRandNum(100, "Minor Civ AI: Decide if we give a unit to the meeting player") < iUnitGift) {
                                        ...
                                        CvUnit* pUnit = DoSpawnUnit(ePlayer, false, true); // serp: since every team member has a chance to get a unit now, spawn it at the player, not at the citystate. we might change this back, if we only do this for the meeting player...
										if (pUnit != NULL) {

	...
	for(it = m_CurrentTurnUnits.begin(); it != m_CurrentTurnUnits.end(); it++)
	{
		...
		if(pLoopUnit)
		{
			if(!bNavalOnly || pLoopUnit->getDomainType() == DOMAIN_SEA)
			{
                ...
                // Serp: this does exactly the opposite for some reason, many camps are left undefended, even with no other player near.
				...
				// AMS: To effectively skip all ranged units... serp: this leads to all ranged barbarian units leaving their camp

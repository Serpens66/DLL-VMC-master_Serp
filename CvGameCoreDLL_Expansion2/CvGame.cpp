...
CvGame::CvGame() :
	...
	, m_bFOW(true)
#if defined(MOD_EVENTS_PRE_SAVE_EVENT)
	, m_bSavedOnce(false)
#endif
...
#if defined(MOD_CVM_AUTOSAVE_FIX)
...
#if defined(BUGFIX_AUI_GAME_FIX_MULTIPLAYER_BARBARIANS_SPAWN_AFTER_MOVING)
	if (!isOption(GAMEOPTION_DYNAMIC_TURNS) && !isOption(GAMEOPTION_SIMULTANEOUS_TURNS))
	{
#endif
...
{
#if defined(MOD_EVENTS_PRE_SAVE_EVENT) // Paz - This will fire before Civ5SavedGameDatabase.db serialization into the gamesave file
	if (MOD_EVENTS_PRE_SAVE_EVENT && m_bSavedOnce) // But... running gDLL->GetScriptSystem on initial save causes a game hang, so skip first save ... it also causes hang on other autosaves -.-
	{
		GAMEEVENTINVOKE_HOOK(GAMEEVENT_PreSaveEvent);
	}
#endif

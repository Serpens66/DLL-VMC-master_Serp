...
void CvPlayer::disconnected()
{
	...
	if(isMultiplayer && isHuman() && !isLocalPlayer())
	{
...
#if defined(MOD_AUI_GAME_AUTOPAUSE_ON_ACTIVE_DISCONNECT_IF_NOT_SEQUENTIAL)
		if (!isObserver() && !CvPreGame::isPitBoss())
		{
			if (gDLL->IsPlayerKicked(GetID()))
			{
		...
		if(!isObserver() && (!CvPreGame::isPitBoss() || gDLL->IsPlayerKicked(GetID())))
		{
            ...
            CUSTOMLOG("disconnected, AI takes over");
...
#if defined(MOD_AUI_GAME_AUTOPAUSE_ON_ACTIVE_DISCONNECT_IF_NOT_SEQUENTIAL)
			else if (isAlive() && isTurnActive() &&
				(GC.getGame().isOption(GAMEOPTION_DYNAMIC_TURNS) || GC.getGame().isOption(GAMEOPTION_SIMULTANEOUS_TURNS)) && !gDLL->IsPlayerKicked(GetID()))
			{
				CUSTOMLOG("disconnected, pause and wait for reconnect");
                for (int iI = 0; iI < MAX_MAJOR_CIVS; iI++)
                {
                    CvPlayer& kPlayer = GET_PLAYER((PlayerTypes)iI);
                    if (kPlayer.isAlive() && kPlayer.isHuman() && !kPlayer.isDisconnected())
                    {
                        DLLUI->AddMessage(0, kPlayer.GetID(), false, GC.getEVENT_MESSAGE_TIME(), "Player disconnected, pause the game until everyone reconnect or close...");
                    }
                }
...
void CvPlayer::reconnected()
{
	...
	if(isMultiplayer && !isLocalPlayer())
	{
...
#if defined(MOD_AUI_GAME_AUTOPAUSE_ON_ACTIVE_DISCONNECT_IF_NOT_SEQUENTIAL)
		CUSTOMLOG("reconnected, should unpause")
        ...
        // -> should be the case, but it is not... unpausing it here will unfortunately trigger the message to host, that he is last active player, but he is not, so ignore this message.
#endif
	}
}
//	-----------------------------------------------------------------------------------------------

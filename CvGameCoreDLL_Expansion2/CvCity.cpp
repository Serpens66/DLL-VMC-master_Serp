...
#if defined(MOD_RESOURCES_WONDERMOD_TXT_OBSOLETE)
                // add obsolete Era information to the notification. added by Serp
                CvResourceInfo* pkResource = GC.getResourceInfo(eResource);
                int iWonderMod = pkResource->getWonderProductionMod();
                EraTypes eObsoleteEra = pkResource->getWonderProductionModObsoleteEra();
                Localization::String strEraText = Localization::Lookup("TXT_KEY_NO_ERA"); // a new txt key, should translate to sth like "unlimited"
                if (eObsoleteEra != NO_ERA)
                {
                    strEraText = GC.getEraInfo(eObsoleteEra)->getShortDesc();
                }
                if(iWonderMod != 0)
				{
					CvNotifications* pNotifications = GET_PLAYER(getOwner()).GetNotifications();
					if(pNotifications)
					{
						// CUSTOMLOG("Add Obs. Era to wondermod ressouce notificaton: %s",strEraText)
                        Localization::String strText = Localization::Lookup("TXT_KEY_NOTIFICATION_CITY_RESOURCE_WONDER_MOD");
						strText << getNameKey() << pkResource->GetTextKey() << iWonderMod << strEraText;
...
void CvCity::DoTestResourceDemanded()
{
	...
	else
	{
		if(eResource != NO_RESOURCE)
		{
			...
			{
#if defined(MOD_NQ_WLTKD_SCALES_BY_GAME_SPEED)
...
int CvCity::getProductionNeeded(UnitTypes eUnit) const
{
	...
	if (eUnit != NO_UNIT)
	{
        ...
        iNumProductionNeeded = max(1,iNumProductionNeeded);
	}
#endif
